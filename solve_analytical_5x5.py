...
def create_window(player_board, field):
    window = [['_' for _ in range(5)] for _ in range(5)]
    half_size = 5 // 2
    row, col = field
...
def gameloop_analytical_5x5(size, default_mines, rand_mines, limits, filename):
...
def simulation_5x5(size, default_mines, rand_mines, limits, filename, seed, iterations):
    ...
    for _ in range(iterations):
        ...
        result = gameloop_analytical_5x5(size, default_mines, rand_mines, limits, filename)
        if result == '?':
            undecided += 1
        elif result == 'W':
            wins += 1
...
if __name__ == '__main__':
    ...
    simulation_5x5(SIZE, DEFAULT_MINES, RAND_MINES, LIMITS, FILENAME, SEED, ITERATIONS)
